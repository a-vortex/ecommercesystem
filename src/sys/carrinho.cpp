...
void Carrinho::adiciona_produto(Produto produto, unsigned quantidade){
    bool existe=false;
...
void Carrinho::remove_produto(Produto produto, unsigned quantidade){
...
std::list<std::pair<Produto, unsigned>> Carrinho::GetList() const{
    return lista_ids_produtos;
}

float Carrinho::GetTotalPrice() const{
    return preco_total;
}

unsigned Carrinho::GetProductQuantity() const{
